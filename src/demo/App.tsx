...
function App() {
  ...
  const initialPrices = TransformD3.sort(
    [0, ...chartData.map((item: any) => item.initial_price)],
    "asc"
  );
  ...
  const dateScale = ScalesD3.utc(TransformD3.extent(dates), [
    settings.marginLeft,
    (settings.width || 0) - settings.marginRight,
  ]);
    ...
    <div id="app">
      ...
      >
        <ChartD3.Bar
          ...
          config={settings}
        />
        <ChartD3.Pie data={initialPrices.slice(1, 8)} config={settings} />
        <ChartD3.Line
          data={chartData}
          field={{
            x: "date",
