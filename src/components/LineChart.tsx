...
const LineChart = ({ data, field, scale, config, style }: LineChartProps) => {
  const { x, y } = scale;
  ...
  useEffect(() => {
    ...
    const line = d3.line(
      (d: any) => {
        if (x.type === "utc") return x.scale(new Date(d[field.x]));
        else return x.scale(d[field.x]);
      },
      (d: any) => {
        if (y.type === "utc") return y.scale(new Date(d[field.y]));
        else return y.scale(d[field.y]);
      }
    );
